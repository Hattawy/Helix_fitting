...
float   kangle[MAX_HITS_ON_CHAIN];
...
//nkb added these variables for the recalculation of dz/ds
 float   my_phi,     xc,        yc,   xdca, small_s;
 float   ydca, xbar, ybar, xpt, ypt, alpha, beta;
 int neg, pos, fix_sxy, first_hit_index; 
if (npt <= 2) 
 {
   //fprintf(stderr,"Function rwfthc(): Cannot fit less than 3 points; exiting..\n");
  errors[1]++;
  return;
 }
if (npt > MAX_HITS_ON_CHAIN) 
 {
   //fprintf(stderr,"Function rwfthc(): Cannot fit more than %d points; exiting..\n", 
   //       MAX_HITS_ON_CHAIN);
  errors[0]++;
  return;
 }
...
den = sqrtf(den2); cur = det*den + 0.0000000001;
alf = -(xm*det + x1)*den;
bet = -(ym*det + y1)*den;
rm = xm*xm + ym*ym;
...
sa2b2 = 1.0/(sqrtf(alf*alf + bet*bet));
...
//we only use this method if the argument of the arcsin is out of range
     ...
     *helix_error = 0;
	 ...
	 sxy[i] = (1/rr0)*sxy[i];
         ...
         if(neg > 0 && pos > 0 && i!=n-1) fix_sxy = 1;//*helix_error = 1;
   ...
   if(fix_sxy)
     {             
       *helix_error = 1;
       /*printf("Fixing dzds calc.....r = %f, first_hit = %d\n",rr0, first_hit_index);
       for(i=0; i<n; i++)
	 {
	   
	   printf("angle[%d] = %f deg ",i,kangle[i]);
	   printf("initial sxy[%d] = %f\n",i,sxy[i]);
	 }
       */
       for(i=0; i<n; i++)
	 {
       ...
       for(i=0; i<n-1; i++)
         {
	   if(sxy[first_hit_index] < 0)
	     {
	       if(sxy[i] > 0) sxy[i] -= 2*PI;
	     }
	   else if(sxy[first_hit_index] > 0)
	     {
	       if(sxy[i] < 0) sxy[i] += 2*PI;
	     }
	   else {//do nothing 
	   }//printf("could not fix.\n");
	 }
...
for(i=0; i<n; i++)
 {
  ...
  if (ff0 < -0.9999)  ff0 = -0.9999; 
  eta = ss0[i] / sqrtf(fabs(1.0+ff0)*(1.0-ff0));
  ...
  ggg = eee[i] / sqrtf(fabs( (1.0+eee[i])*(1.0-eee[i])));
  dza = sxy[i];
...
for(i=0; i<n; i++)
 {
  ...
  if (deln[i] < -ONEPI) deln[i] += TWOPI;
  eee[i] = 0.5*vv1[0]*sqrtf(fabs( (rf[i]*rf[i] - vv1[3]*vv1[3]) / 
           (1.0 - aa0*vv1[0]*vv1[3]) ));
